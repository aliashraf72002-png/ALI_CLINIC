...
const Booking: React.FC = () => {
  ...
  const renderStep1 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <h2 className="text-2xl font-bold text-gray-900">{t('selectService')}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {SERVICES.map((service) => (
          <button
            key={service.id}
            onClick={() => setFormData({ ...formData, serviceId: service.id })}
            className={`p-6 rounded-2xl border-2 text-left transition-all ${formData.serviceId === service.id
                ? 'border-[#228B22] bg-[#228B22]/5 shadow-md'
                : 'border-gray-100 hover:border-gray-200'
              }`}
          >
            <p className="font-bold text-lg text-gray-900">{language === 'ar' ? service.nameAr : service.nameEn}</p>
            <p className="text-sm text-[#228B22]">{language === 'ar' ? service.nameEn : service.nameAr}</p>
          </button>
        ))}
      </div>
      <button
  ...
  const renderStep2 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <h2 className="text-2xl font-bold text-gray-900">{t('chooseTime')}</h2>
      <div className="space-y-4">
        <label className="block text-sm font-bold text-gray-700">{t('preferredDate')}</label>
      ...
      <div className="space-y-4">
        <label className="block text-sm font-bold text-gray-700">{t('availableSlots')}</label>
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
          {timeSlots.map((slot) => (
            <button
              key={slot}
              onClick={() => setFormData({ ...formData, time: slot })}
              className={`py-3 rounded-lg text-sm font-bold transition-all ${formData.time === slot
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <h2 className="text-2xl font-bold text-gray-900">{t('patientDetails')}</h2>
  ...
  const renderSuccess = () => (
    <div className="text-center space-y-8 py-12 animate-in zoom-in-95 duration-500">
      ...
      <div className="space-y-4">
        <h2 className="text-3xl font-extrabold text-gray-900">{t('appointmentRequested')}</h2>
        <p className="text-gray-500 max-w-sm mx-auto">
          {t('requestReceived')} <span className="font-bold">{language === 'ar' ? (SERVICES.find(s => s.id === formData.serviceId)?.nameAr) : (SERVICES.find(s => s.id === formData.serviceId)?.nameEn)}</span> {t('on')} <strong>{formData.date}</strong> {t('at')} <strong>{formData.time}</strong>.
        </p>
      </div>
      <div className="bg-gray-50 p-6 rounded-2xl text-left max-w-sm mx-auto border border-gray-100">
        <h3 className="font-bold text-sm uppercase text-gray-400 mb-4 tracking-widest">{t('nextSteps')}</h3>
        ...
        {step < 4 && (
          <div className="mb-12">
            <div className="flex justify-between items-center mb-4">
              <span className="text-[#228B22] font-bold">{t('step')} {step} {t('of')} 3</span>
              <span className="text-gray-400 font-medium">
                {step === 1 ? t('selectTreatment') : step === 2 ? t('chooseTime') : t('patientDetails')}
              </span>
            </div>
